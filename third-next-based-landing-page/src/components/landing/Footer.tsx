    ...
    <footer className="bg-slate-950/80 backdrop-blur-xl border-t border-white/5">
      <div className="max-w-5xl mx-auto px-4 pt-12 pb-12">
        <div className="flex flex-col items-center gap-4 pb-6">
          ...
          <Link href={home} className="inline-flex opacity-90 transition-opacity hover:opacity-100" aria-label="ET Logic — home">
            <Image src="/images/et-logic-logo-svg.svg" alt="ET Logic" width={148} height={99} className="h-10 w-auto" />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center justify-items-center sm:justify-items-stretch text-center sm:text-left">
          <div className="sm:justify-self-start space-y-1">
            ...
            <a href={privacyPath(locale)} className="text-xs text-white/40 hover:text-white/70 transition-colors">
              {content.privacyLabel}
            </a>
          </div>
          <p className="text-xs text-white/40 order-last sm:order-none sm:justify-self-center">{content.tagline}</p>
          <div className="flex items-center justify-center gap-2 sm:justify-self-end">
            {socialLinks.map((item) => (
              <a key={item.label} href={item.href} aria-label={item.label} className="flex items-center justify-center w-9 h-9 rounded-lg text-white/50 hover:text-white hover:bg-white/5 transition-colors">
